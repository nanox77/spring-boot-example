...
public class UpdateSuperHeroTest {
    ...
    @Test
    public void whenIdParamIsInvalidThenThrowException() {
        UpdateSuperHero updateSuperHero = new UpdateSuperHero(superHeroRepository);

        Exception exception = Assert.assertThrows(InvalidInputException.class, () -> updateSuperHero.execute(null, NEW_SUPERMAN));

        Assert.assertTrue(exception.getMessage().contains("Id is required"));
    }

    @Test
    public void whenNameParamIsInvalidThenThrowException() {
        UpdateSuperHero updateSuperHero = new UpdateSuperHero(superHeroRepository);

        Exception exception = Assert.assertThrows(InvalidInputException.class, () -> updateSuperHero.execute(ID, null));
